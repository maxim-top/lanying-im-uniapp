...
<view class="container">
  ...
  <view class="item nb">
    ...
    <view class="sright">
      ...
      <button v-if="!binded" class="bindBtnClass" open-type="getUserInfo" @getuserinfo="bindGetUserInfo">
        点击绑定
      </button>
    </view>
  </view>
...
export default {
  ...
  methods: {
    ...
    getIsBind() {
      getApp().getIM().sysManage.asyncWechatIsbind().then(binded => {
        this.setData({
          binded
        });
      });
    ...
    unbindHandler() {
      getApp().getIM().sysManage.asyncWechatUnbind().then(res => {
        this.setData({
          binded: false
        });
        this.wxAuth();

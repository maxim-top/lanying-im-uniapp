...
<view class="container" :style="'padding-top:' + navHeight + 'px'">
  <view class="inputFrame">
    <input type="text" :value="name" placeholder="输入群名称" @input="nameHandler"></input>
  </view>
  <view class="inputFrame th mt">
    <input class="th" type="textarea" :value="description" placeholder="输入群描述" @input="descriptionHandler"></input>
  </view>
  <view class="stxt">
    <text>选择群成员</text>
  </view>
  <view class="roster_list">
    <view v-for="(item, index) in rosterList" :key="index" :data-idx="index" class="item" @tap="tapRoster">
...
export default {
  ...
  methods: {
    ...
    create() {
      ...
      getApp().getIM().groupManage.asyncCreate({
        name: this.name,
        type: 1,

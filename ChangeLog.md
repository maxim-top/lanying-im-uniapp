...
### v3.0.6 20240222
1.uniapp 小程序及H5新增账号切换功能，可以在已经登陆过的多个账号直接进行登录切换。
2.修复onRosterInfoUpdate处理时rosterId操作错误导致的设置当前用户自己的别名的问题。
3.增加asyncUpdateRosterAlias接口。
4.使用 用户别名 > 用户昵称 > 用户名 > 用户id的展示顺序来展示用户名。
5.在群组内使用 用户别名 > 用户群名片 > 用户昵称 > 用户名 > 用户id的展示顺序来展示用户名。
6.修复groupInfoUpdate处理群成员昵称变更时误将变更处理群组信息而不是具体的群成员信息的问题。
7.群聊profile界面新增展示群成员列表功能界面。
8.群聊内增加发送@消息的功能。
